...
from src.schema.filter import Filter, boolean_operators, datetime_operators, numeric_operators, textual_operators
...
def safe_bot_session_check(bot, session_id):
    """
    Vérifie de manière sécurisée si une session existe
    Compatible avec différentes versions de besser-bot-framework
    """
    
    # Méthode 1: get_session (si disponible)
    if hasattr(bot, 'get_session'):
        try:
    ...
    # Méthode 2: sessions dict
    if hasattr(bot, 'sessions'):
        try:
            return session_id in bot.sessions
        except Exception:
            pass
    
    # Méthode 3: _sessions dict privé
    if hasattr(bot, '_sessions'):
        try:
            return session_id in bot._sessions
        except Exception:
            pass
    
    # Par défaut, considérer que la session n'existe pas
    return False
...
def playground():
    ...
    app = get_app()
    if DASHBOARD_TAB_SWITCH not in st.session_state:
        st.session_state[DASHBOARD_TAB_SWITCH] = False
    ...
    bot_col, dashboard_col = st.columns([BOT_CONTAINER_WIDTH, 1 - BOT_CONTAINER_WIDTH])
    ...
    with dashboard_col:
        ...
        if project:
            ...
            if selected_tab == 0:  # Data
                ...
                if session_id and safe_bot_session_check(project.databot.bot, session_id):
