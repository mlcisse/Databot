...
class DataBot:
    ...
    def _set_bot_properties(self):
        api_key = self.project.app.properties[session_state_keys.OPENAI_API_KEY]
        print(f"DEBUG: API Key from app properties: {api_key}")
        print(f"DEBUG: API Key type: {type(api_key)}")
        print(f"DEBUG: API Key is None: {api_key is None}")
        self.bot.set_property(OPENAI_API_KEY, api_key)
